...
class AddProduct(StatesGroup):
    ...
    texts = {
        ...
        'AddProduct:image': 'Отправьте картинку заново',
    }
...
async def delete_product(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession):
    product_id = callback.data.split("_")[-1]
    ...
    await callback.message.answer(
        'Введите название товара. Введите "." если хотите пропустить шаг, введите "отмена для отмены ввода "', reply_markup=types.ReplyKeyboardRemove()
    )
...
async def add_product(message: types.Message, state: FSMContext):
    await message.answer(
        'Введите название товара, введите "отмена для отмены ввода "', reply_markup=types.ReplyKeyboardRemove()
    )
    await state.set_state(AddProduct.name)
...
async def add_description(message: types.Message, state: FSMContext):
    if message.text == "." and AddProduct:
...
async def add_image(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await message.answer(
            f"Ошибка: \n{str(e)}\nОбратитесь к разработчику. Tg: @Bbylba4ka"
        )
        await state.clear()
